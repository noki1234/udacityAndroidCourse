...
class GameWonFragment : Fragment() {
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
        ...
        setHasOptionsMenu(true) //1: Add option menu
    ...
    override fun onCreateOptionsMenu(menu: Menu, inflater: MenuInflater) { //2: Override method onCreateOptionsMenu
        ...
        if (null == getShareIntent().resolveActivity(requireActivity().packageManager)) { //in case there are no others activities that support our
                                                                                        // text sharing app will crash - in this case we will hide button share
            // hide the menu item if it doesn't resolve
            menu.findItem(R.id.share)?.isVisible = false
        }
    }

    //3: Create a getShareIntent method
...
//        return shareIntent
    ...
    private fun shareSuccess() { // 4: Create a shareSuccess() method
